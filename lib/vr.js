...
vr.Runtime.prototype.createEmbed_ = function() {
  ...
  // The embed object doesn't seem to work unless it takes
  // up space on the screen.  To minimize interference with
  // scrolling, it's fixed to the top-left corner, invisible,
  // but with nonzero size.
  ...
  embed.style.top = '0';
  embed.style.left = '0';
  embed.style.width = '4px';
  embed.style.height = '4px';
  embed.style.margin = '0';
  embed.style.padding = '0';
  embed.style.borderStyle = 'none';
  embed.style.borderWidth = '0';
  embed.style.maxWidth = '4px';
  embed.style.maxHeight = '4px';
  return embed;
};
 ...
 * @private
 */
 // hawksley: set to 0 because not an issue with video, and plays much faster
vr.StereoRenderer.RENDER_TARGET_SCALE_ = 1;
...
vr.StereoRenderer.prototype.initialize_ = function() {
  ...
  this.setupRenderTarget_(
      gl.canvas.width * vr.StereoRenderer.RENDER_TARGET_SCALE_,
      gl.canvas.height * vr.StereoRenderer.RENDER_TARGET_SCALE_);
...
vr.StereoRenderer.prototype.renderEye_ = function(eye) {
  ...
  var fullWidth = gl.canvas.width;
  var fullHeight = gl.canvas.height;
